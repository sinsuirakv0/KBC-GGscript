...
キャラ・形態・操作の選択一覧は、最後に選んだ位置を記憶して開き直す。ステータス変更は117項目の数値入力欄とチェックボックスを一つのスクロール画面に表示し、確認時に変更された項目だけを反映する。項目間の区切り線は表示しない。画面の先頭と末尾にある「戻る」は変更を適用せずに前の操作画面へ戻るためのチェック欄である。
