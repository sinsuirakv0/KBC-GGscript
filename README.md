...
フォーム選択後は「キャラ解放/レベル変更」「ステータス変更」「リストに保存」「戻る」を表示する。「キャラ解放/レベル変更」にはキャラ解放、キャラ削除、レベル変更がある。削除時は確認画面を表示し、ネコは削除できない。レベル変更では現在値を初期表示してレベルとプラス値を入力できる。凍結は初期状態がオンで、オフにすると同じレベルアドレスの既存凍結を解除して値だけを書き込む。保存領域を検証できない場合は書き込まず、候補数の上限で探索を終了してエラーを表示する。

実行中にLuaやキャラ情報をネットワークから取得しないため、端末へリポジトリの`data`と`lua`をコピーした後は完全オフラインで利用できる。

キャラ・形態・操作の選択一覧は、最後に選んだ位置を記憶して開き直す。ステータス変更は117項目の数値入力欄とチェックボックスを一つのスクロール画面に表示し、確認時に変更された項目だけを反映する。項目間の区切り線は表示しない。先頭の「戻る」の直下にある「全てリセット」を選ぶと確認画面を表示し、「はい」で選択中のキャラ・形態の117項目を同梱CSVの初期値へ戻す。画面の先頭と末尾にある「戻る」は変更を適用せずに前の操作画面へ戻るためのチェック欄である。
