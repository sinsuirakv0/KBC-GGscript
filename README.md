...
フォーム選択後は「キャラ解放/レベル変更」「ステータス変更」「リストに保存」「戻る」を表示する。キャラ解放は選択キャラの所持状態を変更し、レベル変更では現在値を初期表示してレベルとプラス値を入力できる。凍結は初期状態がオンで、オフにすると同じレベルアドレスの既存凍結を解除して値だけを書き込む。保存領域を検証できない場合は書き込まない。
